...
### 2.3 `evidence-manifest-svc` never checks legal-hold state before exporting
`ZS-SVC-S-001` (Document/Record/Retention/Legal-Hold), DRC-I10: "legal hold blocks ordinary disposition for every resolved target." `evidence-manifest-svc`'s `ScenarioType` enum (AUDIT/REGULATOR/LEGAL_DISCOVERY/COMPLIANCE_REVIEW) targets exactly the use cases this invariant governs, but the service has no legal-hold check against `retention-registry-svc` before assembling a manifest. Nothing currently stops exporting a record that should be blocked by an active hold.

### 2.4 `retention-registry-svc`'s `GetLegalHold` has zero authorization (self-documented gap from earlier work)
Confirmed by the QA/data-governance doc review — the code comment already says this was deliberately deferred rather than invented. Small, fixable, sitting right next to the Priority 2b work already done.
...
### 2.6 `authorization-svc` has no dynamic Segregation-of-Duties layer at all
`ZS-IAM-001` §10.2 requires dynamic SoD checks — e.g., a preparer cannot approve their own object (`resource.preparer_id == subject_id AND action == approve → DENY`). Current `authorization-svc` only checks static action-pairs held simultaneously by a principal; it has no relationship/ownership predicate evaluation and no `resource_attributes.preparer_id`-equivalent input at all. This is described as a mandatory, non-optional layer in the new doc — not deferred ABAC.
