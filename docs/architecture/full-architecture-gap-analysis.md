...
| # | Issue | Status |
|---|---|---|
| 1 | ~~No shared/enforced event envelope — every service hand-copies its own struct; `event_version` and jurisdiction context are missing from almost every event; `tenant_id`/`legal_entity_id`/`actor_id` present only when that call site happened to include them~~ | **Resolved (2026-08-20)** — no shared Go module exists to enforce this structurally, so each service still hand-copies its own envelope; what's now complete is that every one of the ~90 services on the platform has a real, verified, correctly-populated Doc 03 §19 envelope (event_id, event_version, timestamp, tenant_id, legal_entity_id, jurisdiction where the domain object genuinely has one, actor_id, correlation_id, source_service, payload schema_version), with every genuinely-absent field `omitempty`'d and documented rather than fabricated. The pass surfaced and fixed a long tail of real, independent bugs along the way: deterministic `event_id` collisions (21+ services) that any `ON CONFLICT (event_id) DO NOTHING` dedup consumer would have silently dropped a repeat real event on; 6 services whose `Publish` logged "event published" but never called `WriteMessages` at all; 5 services publishing via a detached goroutine against `context.Background()` with an unconditional `return nil`, making a broker outage invisible to the caller; several services discarding an already-authz-verified principal instead of using it as actor_id. See PR #110 (`dev` → `main`) for the full service-by-service breakdown. |
