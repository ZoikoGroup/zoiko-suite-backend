...
| 11b | commercial-account-svc — RLS + subscription-store scoping | Not Started | The backstop half. (a) No RLS migration exists at all. Three table classes: `commercial_accounts`/`memberships` scope **directly** on `organization_id`; the catalog (`price_catalogs`, `plans`, `entitlement_limits`) is correctly **platform-scope** with no tenant column (doc7 §U1/§B1 — every tenant reads the same approved catalog); and 7 tables scope **indirectly** via `commercial_account_id` → `commercial_accounts.organization_id`, needing either a subquery policy or a denormalized column. Note the subquery variant makes the children's isolation depend on the parent's policy — removing the parent policy silently widens the children, which is worth its own negative control. (b) All of `subscription_store.go` is unscoped (`GetSubscription`, `ListStatusEventsBySubscription`, `GetEvaluationProgramBySubscription`, `GetChangeRequest`, `ResolveEntitlement`, and the writes `TransitionSubscriptionStatus`/`UpdateSubscriptionPlan`/`ApplyChangeRequest`/`CreateBillingSourceTransfer`). Gated today by handler authz against the resource's own account, so not an open door, but the store must not depend on handler ordering. (c) Must include a **store-level** test calling a tenant-scoped read with a tenant-less context — the one thing 11a's handler tests provably cannot cover. |
