...
type Validator interface {
	...
	TerminateEmployee(ctx context.Context, principalID, employeeID string) error
}
...
func (c *Client) TerminateEmployee(ctx context.Context, principalID, employeeID string) error {
	if c.baseURL == "" {
		return nil
	}

	url := fmt.Sprintf("%s/v1/employees/%s/terminate", c.baseURL, employeeID)
	...
	resp, err := c.httpClient.Do(req)
	if err != nil {
		return fmt.Errorf("employee-master-svc unreachable: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK && resp.StatusCode != http.StatusNoContent {
		return fmt.Errorf("employee-master-svc termination status returned %d", resp.StatusCode)
	}
	return nil
}
