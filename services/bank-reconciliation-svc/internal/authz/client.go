...
// that a principal is actually allowed to perform a given bank
// reconciliation action.
//
// Doctrine (03-microservices.md): no service self-authorizes a material
// action. The Match/FlagException/CompleteStatement actions are checked
// against authorization-svc synchronously, fail-closed — an unreachable
// authorization-svc rejects the action, it never silently permits it. This
// is financial data, so this client is wired for real from day one (unlike
// some Phase 1 services that shipped with a documented stub-first posture).
package authz
