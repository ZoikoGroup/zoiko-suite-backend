...
// service, and payload schema version. Neither domain.WageRevision nor
// domain.BonusGrant has a legal_entity_id, jurisdiction, or actor field of
// its own — the employee's legal entity is resolved at each handler call
// site (via resolveEmployeeEntity) and threaded through explicitly,
...
func (p *Publisher) PublishCompensationUpdated(ctx context.Context, correlationID, legalEntityID, actorID string, rev domain.WageRevision) {
	p.emit(ctx, "compensation.updated", correlationID, rev.TenantID, legalEntityID, actorID, rev.RevisionID, map[string]any{
...
func (p *Publisher) PublishEffectiveChanged(ctx context.Context, correlationID, legalEntityID, actorID string, rev domain.WageRevision) {
	p.emit(ctx, "compensation.effective.changed", correlationID, rev.TenantID, legalEntityID, actorID, rev.RevisionID, map[string]any{
		"revision_id":    rev.RevisionID,
		"tenant_id":      rev.TenantID,
		"employee_id":    rev.EmployeeID,
...
func (p *Publisher) emit(ctx context.Context, eventType, correlationID, tenantID, legalEntityID, actorID, key string, payload map[string]any) {
	...
	env := envelope{
		...
		SchemaVersion: "1.0",
		SourceService: "compensation-svc",
		TenantID:      tenantID,
