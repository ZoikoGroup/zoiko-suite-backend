...
// Tenant-scope tests for reconciliation-intelligence-svc (Priority 1b).
//
...
func TestMissingTenantHeader_Refused(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"Post /analyze", http.MethodPost, "/v1/reconciliations/analyze"},
		{"Get /", http.MethodGet, "/v1/reconciliations/"},
		{"Get /some-id", http.MethodGet, "/v1/reconciliations/some-id"},
		{"Post /some-id/resolutions/i1/apply", http.MethodPost, "/v1/reconciliations/some-id/resolutions/i1/apply"},
		{"Delete /some-id", http.MethodDelete, "/v1/reconciliations/some-id"},
	} {
