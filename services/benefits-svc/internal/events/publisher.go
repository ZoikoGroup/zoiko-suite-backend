...
// service, and payload schema version. domain.BenefitElection has no
// legal_entity_id, jurisdiction, or actor field of its own — the enrolled
// employee's legal entity is resolved at each handler call site (via
// resolveEmployeeEntity) and threaded through explicitly, alongside the
// already-verified principalID as actor_id.
...
func (p *Publisher) PublishBenefitEnrolled(ctx context.Context, correlationID, legalEntityID, actorID string, e domain.BenefitElection) {
	p.emit(ctx, "benefit.enrolled", correlationID, e.TenantID, legalEntityID, actorID, e.ElectionID, map[string]any{
...
func (p *Publisher) PublishBenefitChanged(ctx context.Context, correlationID, legalEntityID, actorID string, e domain.BenefitElection) {
	p.emit(ctx, "benefit.changed", correlationID, e.TenantID, legalEntityID, actorID, e.ElectionID, map[string]any{
		"election_id":                  e.ElectionID,
		"tenant_id":                    e.TenantID,
		"employee_id":                  e.EmployeeID,
		"plan_id":                      e.PlanID,
...
func (p *Publisher) PublishBenefitTerminated(ctx context.Context, correlationID, legalEntityID, actorID string, e domain.BenefitElection) {
	p.emit(ctx, "benefit.terminated", correlationID, e.TenantID, legalEntityID, actorID, e.ElectionID, map[string]any{
		"election_id":   e.ElectionID,
		"tenant_id":     e.TenantID,
		"employee_id":   e.EmployeeID,
		"plan_id":       e.PlanID,
