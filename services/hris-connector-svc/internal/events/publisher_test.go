...
// domain.HrisIntegration carries a real LegalEntityID; domain.SyncJob does
// not, so legal_entity_id is correctly omitted for sync events.
package events_test
...
func TestPublish_ActuallyWritesToKafka(t *testing.T) {
	w := &fakeWriter{}
	p := events.NewKafkaPublisherWithWriter(w, "zoiko.hris-connector.events", zap.NewNop())

	err := p.Publish(context.Background(), events.PublishParams{
		EventType: "hris.integration.created", AggregateID: "integ-1", TenantID: "tenant-1",
	...
	assert.Equal(t, "hris.integration.created", env.EventType)
	assert.Equal(t, "1.0", env.EventVersion)
	assert.Equal(t, "hris-connector-svc", env.SourceService)
...
func TestPublish_RepeatEventsOnSameAggregate_GetDistinctEventIDs(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		err := p.Publish(context.Background(), events.PublishParams{
			EventType: "hris.sync.triggered", AggregateID: "job-1", TenantID: "tenant-1",
