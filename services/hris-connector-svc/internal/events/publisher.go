...
// service, and payload schema version. domain.HrisIntegration carries a
// real LegalEntityID; domain.SyncJob does not (it links back to an
// integration by ID only), so legal_entity_id is correctly omitted for
// sync events. Neither domain object has a jurisdiction field or an
...
func (p *KafkaPublisher) Publish(ctx context.Context, params PublishParams) error {
	evt := Event{
		...
		EventVersion:  "1.0",
		SourceService: "hris-connector-svc",
