...
// fabricated: report metric definitions are platform-wide reference data —
// domain.ReportMetricDefinition has no tenant_id field at all, same
// platform-scoped pattern already established for capability-registry-svc
// and jurisdiction-rules-svc.
type Event struct {
...
func (p *KafkaPublisher) Publish(ctx context.Context, params PublishParams) error {
	evt := Event{
		...
		EventVersion:  "1.0",
		SchemaVersion: "1.0",
		SourceService: "metric-registry-svc",
