...
import (
	...
	svcenvelope "zoiko.io/corporate-actions-svc/internal/envelope"
	...
	"zoiko.io/corporate-actions-svc/internal/mtls"
	"zoiko.io/corporate-actions-svc/internal/store"
	"zoiko.io/corporate-actions-svc/internal/telemetry"
)

func main() {
	logger, err := telemetry.NewLogger("corporate-actions-svc")
	if err != nil {
	...
	if cfg.AuthzMTLSEnabled {
		mtlsHTTPClient, err := mtls.NewClientHTTPClient(context.Background(), cfg.MTLSManagementServiceURL, "corporate-actions-svc", "00000000-0000-0000-0000-00000000f001")
		if err != nil {
	...
	go func() {
		logger.Info("starting corporate-actions-svc", zap.String("port", cfg.Port))
	...
	logger.Info("shutting down corporate-actions-svc gracefully...")
