...
import (
	...
	"zoiko.io/compliance-status-svc/internal/config"
	svcenvelope "zoiko.io/compliance-status-svc/internal/envelope"
	"zoiko.io/compliance-status-svc/internal/events"
	"zoiko.io/compliance-status-svc/internal/handler"
	"zoiko.io/compliance-status-svc/internal/health"
	"zoiko.io/compliance-status-svc/internal/middleware"
	"zoiko.io/compliance-status-svc/internal/mtls"
	"zoiko.io/compliance-status-svc/internal/store"
	"zoiko.io/compliance-status-svc/internal/telemetry"
)

func main() {
	logger, err := telemetry.NewLogger("compliance-status-svc")
	if err != nil {
	...
	if cfg.AuthzMTLSEnabled {
		mtlsHTTPClient, err := mtls.NewClientHTTPClient(context.Background(), cfg.MTLSManagementServiceURL, "compliance-status-svc", "00000000-0000-0000-0000-00000000f001")
		if err != nil {
	...
	go func() {
		logger.Info("starting compliance-status-svc", zap.String("port", cfg.Port))
	...
	logger.Info("shutting down compliance-status-svc gracefully...")
