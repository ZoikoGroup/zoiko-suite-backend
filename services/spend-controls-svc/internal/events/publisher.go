...
// service, and payload schema version. domain.SpendPolicy carries real
// TenantID/LegalEntityID but no actor field for a submitted spend check,
// so actor_id is threaded through explicitly from the handler's
...
func (p *Publisher) PublishThresholdBreached(ctx context.Context, correlationID, actorID string, check domain.SpendCheckRequest, policy domain.SpendPolicy, projected float64) {
	p.emit(ctx, "spend.threshold.breached", correlationID, policy.TenantID, check.LegalEntityID, actorID, policy.SpendPolicyID, map[string]any{
...
func (p *Publisher) PublishBlockApplied(ctx context.Context, correlationID, actorID string, check domain.SpendCheckRequest, policy domain.SpendPolicy) {
	p.emit(ctx, "spend.block.applied", correlationID, policy.TenantID, check.LegalEntityID, actorID, policy.SpendPolicyID, map[string]any{
		"spend_policy_id":  policy.SpendPolicyID,
		"legal_entity_id":  check.LegalEntityID,
		"category":         check.Category,
