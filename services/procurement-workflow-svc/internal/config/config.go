...
func Load() (*Config, error) {
	return &Config{
		Env:  env("ENV", "local"),
		Port: envInt("PORT", 8134),
		DB: DBConfig{
			Host:     env("DB_HOST", "localhost"),
			Port:     envInt("DB_PORT", 5432),
			Name:     env("DB_NAME", "procurement_workflow"),
		...
		Kafka: KafkaConfig{
			...
			GroupID: env("KAFKA_GROUP_ID", "procurement-workflow-svc"),
			Topic:   env("KAFKA_EVENTS_TOPIC", "zoiko.procurement-workflow.events"),
		},
