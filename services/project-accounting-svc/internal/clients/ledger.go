...
// asset-management-svc's and inventory-management-svc's own
// clients.LedgerLine.
...
// PostRecognitionAccountingEvent is PRJ-03's own "ACC-04" dependency —
...
// path, "Recognition rerun duplicates GL posting."
func (c *Clients) PostRecognitionAccountingEvent(ctx context.Context, tenantID, principalID, legalEntityID, fiscalPeriod, description, sourceEventID, correlationID string, lines []LedgerLine) (journalID string, err error) {
	...
	if err != nil {
		c.log.Error("failed to post recognition accounting event", zap.Error(err))
...
// ReverseRecognitionJournal calls general-ledger-svc's own
// CreateReversalPosting — the same reversal primitive every other posted
// journal on this platform uses.
func (c *Clients) ReverseRecognitionJournal(ctx context.Context, tenantID, principalID, journalID, reason string) error {
	...
	resp, err := c.http.Do(req)
	if err != nil {
		c.log.Error("failed to reverse recognition journal", zap.Error(err))
		return domain.ErrStoreUnavailable
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK && resp.StatusCode != http.StatusCreated {
