...
--
-- Row-level security on data_feed_subscriptions and data_feed_events. Both carry
...
ALTER TABLE data_feed_subscriptions ENABLE ROW LEVEL SECURITY;
ALTER TABLE data_feed_subscriptions FORCE ROW LEVEL SECURITY;
CREATE POLICY tenant_isolation_policy ON data_feed_subscriptions
    FOR ALL
    USING (tenant_id = NULLIF(current_setting('app.tenant_id', true), ''))
    WITH CHECK (tenant_id = NULLIF(current_setting('app.tenant_id', true), ''));

ALTER TABLE data_feed_events ENABLE ROW LEVEL SECURITY;
ALTER TABLE data_feed_events FORCE ROW LEVEL SECURITY;
CREATE POLICY tenant_isolation_policy ON data_feed_events
    FOR ALL
    USING (tenant_id = NULLIF(current_setting('app.tenant_id', true), ''))
    WITH CHECK (tenant_id = NULLIF(current_setting('app.tenant_id', true), ''));
