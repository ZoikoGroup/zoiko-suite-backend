...
// service, and payload schema version. domain.DataFeedSubscription
// carries a real LegalEntityID; domain.DataFeedEvent does not (it links
// back to a feed by ID only), so legal_entity_id is correctly omitted for
// ingestion events. Neither domain object has a jurisdiction field or an
...
func (p *KafkaPublisher) Publish(ctx context.Context, params PublishParams) error {
	evt := Event{
		...
		EventVersion:  "1.0",
		SourceService: "external-data-feed-svc",
	...
	if p.logger != nil {
		p.logger.Info("event published", zap.String("event_type", params.EventType), zap.String("topic", p.topic))
	}
	return nil
}
