// Package outbox implements the Transactional Outbox pattern for accounts-payable-svc
// per ZS-STATE-001 Invariant I-13.
//
// An event is written to outbox_events inside the SAME database transaction as the
// authoritative vendor invoice mutation, guaranteeing state mutations and event
...
// VariantAEnvelope preserves the existing accounts-payable-svc Kafka event contract.
