...
import (
	...
	"zoiko.io/filing-tracker-svc/internal/config"
	svcenvelope "zoiko.io/filing-tracker-svc/internal/envelope"
	"zoiko.io/filing-tracker-svc/internal/events"
	"zoiko.io/filing-tracker-svc/internal/handler"
	"zoiko.io/filing-tracker-svc/internal/health"
	"zoiko.io/filing-tracker-svc/internal/middleware"
	"zoiko.io/filing-tracker-svc/internal/mtls"
	"zoiko.io/filing-tracker-svc/internal/store"
	"zoiko.io/filing-tracker-svc/internal/telemetry"
)

func main() {
	logger, err := telemetry.NewLogger("filing-tracker-svc")
	if err != nil {
	...
	if cfg.AuthzMTLSEnabled {
		mtlsHTTPClient, err := mtls.NewClientHTTPClient(ctx, cfg.MTLSManagementServiceURL, "filing-tracker-svc", platformScopeID)
		if err != nil {
	...
	go func() {
		logger.Info("starting filing-tracker-svc", zap.String("port", cfg.Port))
	...
	logger.Info("shutting down filing-tracker-svc gracefully...")
