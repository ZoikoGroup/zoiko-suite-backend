...
// Event is the canonical fact emitted by filing-tracker-svc.
...
type Publisher interface {
	Publish(ctx context.Context, eventType, filingID, tenantID string, payload interface{}) error
}
...
func (p *KafkaPublisher) Publish(ctx context.Context, eventType, filingID, tenantID string, payload interface{}) error {
	evt := Event{
		EventID:    "evt-" + eventType + "-" + filingID,
		EventType:  eventType,
		FilingID:   filingID,
		TenantID:   tenantID,
