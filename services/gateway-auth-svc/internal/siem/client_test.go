...
func TestStream_DoesNotBlockTheCaller(t *testing.T) {
	...
	c := siem.New(upstream.URL, "gateway-auth-svc", zap.NewNop())
	defer c.Close()

	start := time.Now()
	c.Stream(context.Background(), testTenant, "tenant_context.denied", siem.SeverityHigh, "denied")
...
func TestStream_DoesNotUseTheRequestContext(t *testing.T) {
	...
	c.Stream(ctx, testTenant, "tenant_context.denied", siem.SeverityHigh, "denied")
...
func TestStream_DisabledStartsNothing(t *testing.T) {
	c := siem.New("", "gateway-auth-svc", zap.NewNop())
...
func TestStream_NoTenantIsANoOp(t *testing.T) {
	...
	c := siem.New(upstream.URL, "gateway-auth-svc", zap.NewNop())
	c.Stream(context.Background(), "", "tenant_context.denied", siem.SeverityHigh, "denied")
	c.Close()
}
