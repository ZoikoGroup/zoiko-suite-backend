...
func TestInsert_SerializesHeadersAndPayload(t *testing.T) {
	...
	e := outbox.Event{
		...
		ActorID:       &actor,
		CorrelationID: "corr-123",
		Headers:       map[string]string{"X-Source": "test-suite"},
		Payload:       map[string]any{"invoice_id": "inv-123", "amount": 1500.0},
	}
	if err := outbox.Insert(context.Background(), tx, e); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if parsedHeaders["X-Source"] != "test-suite" {
		t.Fatalf("expected header X-Source=test-suite, got %v", parsedHeaders)
	}
	...
	if parsedPayload["invoice_id"] != "inv-123" {
		t.Fatalf("expected invoice_id=inv-123 in payload, got %v", parsedPayload)
	}
}

// ── Wire Contract Tests (Variant B) ─────────────────────────────────────────

func TestVariantBEnvelope_EventIDFormatAndPresence(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("failed to construct Variant B envelope: %v", err)
	}
	...
	// CRITICAL: Variant B requires event_id in JSON body formatted as "evt-" + uuid
	eventIDVal, exists := rawMap["event_id"]
	if !exists {
		t.Fatalf("CRITICAL CONTRACT VIOLATION: event_id missing in Variant B JSON body: %s", string(body))
	}
	expectedEventID := "evt-" + rawUUID
	if eventIDVal != expectedEventID {
		t.Fatalf("expected event_id=%s, got %v", expectedEventID, eventIDVal)
	}
}

func TestVariantBEnvelope_WireContractShape(t *testing.T) {
	rawUUID := uuid.NewString()
	env, err := outbox.NewVariantBEnvelope(
		rawUUID,
		"payment.received",
	...
	if env.EventID != "evt-"+rawUUID {
		t.Fatalf("expected event_id=evt-%s, got %s", rawUUID, env.EventID)
	}
	if env.EventType != "payment.received" {
		t.Fatalf("expected event_type=payment.received, got %s", env.EventType)
	}
	...
	if env.SourceService != "accounts-receivable-svc" {
		t.Fatalf("expected source_service=accounts-receivable-svc, got %s", env.SourceService)
	}
	if env.CorrelationID != "corr-999" {
		t.Fatalf("expected correlation_id=corr-999, got %s", env.CorrelationID)
	}
	...
	if env.ActorID != "principal-pay" {
		t.Fatalf("expected actor_id=principal-pay, got %s", env.ActorID)
	}
}
...
func TestRelay_PublishesEventsAndMarksPublishedAt(t *testing.T) {
	...
	env, _ := outbox.NewVariantBEnvelope(
		evtID, "invoice.issued", "c1", "t1", "e1", "p1",
	...
	rows := newMockRows([][]any{
		{
			evtID, "CUSTOMER_INVOICE", "inv-1", "invoice.issued",
			"t1", "e1", actor, "c1", []byte("{}"),
			envJSON, 0,
		},
	})
	...
	if call.aggregateID != "inv-1" {
		t.Fatalf("expected aggregateID=inv-1 (partition key), got %s", call.aggregateID)
	}
...
func TestRelay_IncrementsAttemptsAndRecordsLastErrorOnPublishFailure(t *testing.T) {
	evtID := uuid.NewString()
	env, _ := outbox.NewVariantBEnvelope(
		evtID, "invoice.sent", "c2", "t1", "e1", "p1",
		map[string]any{"invoice_id": "inv-2"},
	)
	envJSON, _ := json.Marshal(env)
	...
	rows := newMockRows([][]any{
		{
			evtID, "CUSTOMER_INVOICE", "inv-2", "invoice.sent",
			"t1", "e1", actor, "c2", []byte("{}"),
			envJSON, 0,
		},
	})
	tx := &mockTx{queryRows: rows}
	pool := &mockPool{tx: tx}
	pub := &mockPublisher{err: errors.New("kafka broker unreachable")}
	log := zap.NewNop()
...
func TestRelay_CrashBeforePublishedAtCommit_EventEligibleForRetryWithSameXEventIDAndJSONEventID(t *testing.T) {
	...
	// allowing downstream consumers to deduplicate using the stable X-Event-ID header and JSON event_id.
	evtID := uuid.NewString()
	env, _ := outbox.NewVariantBEnvelope(
		evtID, "payment.received", "c3", "t1", "e1", "p1",
		map[string]any{"invoice_id": "inv-3"},
	)
	envJSON, _ := json.Marshal(env)
	...
	rows1 := newMockRows([][]any{
		{
			evtID, "CUSTOMER_INVOICE", "inv-3", "payment.received",
			"t1", "e1", actor, "c3", []byte("{}"),
			envJSON, 0,
		},
	})
	...
	// Also verify that the payload's JSON event_id remains identical
	var firstEnv, secondEnv outbox.VariantBEnvelope
	_ = json.Unmarshal(pub.publishes[0].payload, &firstEnv)
	_ = json.Unmarshal(pub.publishes[1].payload, &secondEnv)
	if firstEnv.EventID != secondEnv.EventID || firstEnv.EventID != "evt-"+evtID {
		t.Fatalf("expected stable JSON event_id across redelivery: %s vs %s", firstEnv.EventID, secondEnv.EventID)
	}
}
...
func TestRelay_MaxAttemptsCeiling_QueryFilterAndDeadLetterLogging(t *testing.T) {
	...
	envBytes := []byte(`{"event_type":"customer_invoice.created"}`)

	// Event is on its 9th attempt (Attempts = 9)
	rows := newMockRows([][]any{
		{
			eventID, "CUSTOMER_INVOICE", "inv-deadletter-1", "customer_invoice.created",
...
func TestRelay_DeadLetter_DoesNotBlockSubsequentEvents(t *testing.T) {
	...
	rows := newMockRows([][]any{
		{
			poisonedID, "CUSTOMER_INVOICE", "inv-poison-1", "customer_invoice.created",
			"t-1", "e-1", "a-1", "corr-1", []byte("{}"), envBytes, 0,
		},
		{
			healthyID, "CUSTOMER_INVOICE", "inv-healthy-2", "customer_invoice.created",
