...
import (
	...
	"zoiko.io/clause-template-svc/internal/config"
	svcenvelope "zoiko.io/clause-template-svc/internal/envelope"
	"zoiko.io/clause-template-svc/internal/events"
	"zoiko.io/clause-template-svc/internal/handler"
	"zoiko.io/clause-template-svc/internal/health"
	"zoiko.io/clause-template-svc/internal/middleware"
	"zoiko.io/clause-template-svc/internal/mtls"
	"zoiko.io/clause-template-svc/internal/store"
	"zoiko.io/clause-template-svc/internal/telemetry"
)
...
func main() {
	logger, err := telemetry.NewLogger("clause-template-svc")
	if err != nil {
	...
	if cfg.AuthzMTLSEnabled {
		mtlsHTTPClient, err := mtls.NewClientHTTPClient(ctx, cfg.MTLSManagementServiceURL, "clause-template-svc", platformScopeID)
		if err != nil {
	...
	go func() {
		logger.Info("starting clause-template-svc", zap.String("port", cfg.Port))
	...
	logger.Info("shutting down clause-template-svc gracefully...")
