...
//
// WHAT THIS FIXES. Every publish used to be a goroutine holding a context
// detached from the request, writing straight to Kafka. Three things went
// wrong with that, and only the first was ever visible:
//
//  1. A broker blip dropped the event. The resolution had already succeeded
//     and the caller already held an envelope, so the service reported
//     success while no record of it reached the estate.
//  2. SIGTERM dropped whatever was in flight. Drain bounded the wait, which
//     stopped the process hanging, but a drain that times out still loses the
//     events it was draining.
//  3. The evidence write and the business write could not both succeed or
//     both fail. A session persisted with no event, or an event published for
//     a session that failed to persist, were both reachable states.
//
...
// Delivery is at-least-once, which is what the spec's invariant 10 specifies
// ("outbox publication occurs only after authoritative commit; consumers
