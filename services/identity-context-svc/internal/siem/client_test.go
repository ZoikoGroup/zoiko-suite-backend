...
// siem-integration-svc added its own latency to the request that triggered
// the event — 850ms median against a service that was not running, measured
// on authorization-svc, whose copy of this package was byte-identical to
// this one below the doc comment.
//
...
func TestStream_DoesNotBlockTheCaller(t *testing.T) {
	...
	c := siem.New(upstream.URL, "identity-context-svc", zap.NewNop())
	defer c.Close()

	start := time.Now()
	c.Stream(context.Background(), testTenant, "identity.authentication.failed", siem.SeverityHigh, "denied")
...
func TestStream_DoesNotUseTheRequestContext(t *testing.T) {
	...
	c.Stream(ctx, testTenant, "identity.authentication.failed", siem.SeverityHigh, "denied")
...
func TestStream_DisabledStartsNothing(t *testing.T) {
	c := siem.New("", "identity-context-svc", zap.NewNop())
...
func TestStream_NoTenantIsANoOp(t *testing.T) {
	...
	c := siem.New(upstream.URL, "identity-context-svc", zap.NewNop())
	c.Stream(context.Background(), "", "identity.authentication.failed", siem.SeverityHigh, "denied")
	c.Close()
}
