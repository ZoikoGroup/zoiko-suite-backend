...
--
-- Row-level security on api_bridges and ingestion_logs. Both carry
...
ALTER TABLE api_bridges ENABLE ROW LEVEL SECURITY;
ALTER TABLE api_bridges FORCE ROW LEVEL SECURITY;
CREATE POLICY tenant_isolation_policy ON api_bridges
    FOR ALL
    USING (tenant_id = NULLIF(current_setting('app.tenant_id', true), ''))
    WITH CHECK (tenant_id = NULLIF(current_setting('app.tenant_id', true), ''));

ALTER TABLE ingestion_logs ENABLE ROW LEVEL SECURITY;
ALTER TABLE ingestion_logs FORCE ROW LEVEL SECURITY;
CREATE POLICY tenant_isolation_policy ON ingestion_logs
    FOR ALL
    USING (tenant_id = NULLIF(current_setting('app.tenant_id', true), ''))
    WITH CHECK (tenant_id = NULLIF(current_setting('app.tenant_id', true), ''));
