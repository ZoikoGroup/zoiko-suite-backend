...
// service, and payload schema version. domain.ApiBridge carries a real
// LegalEntityID; domain.IngestionLog does not (it links back to a bridge
// by ID only), so legal_entity_id is correctly omitted for ingestion
// events. Neither domain object has a jurisdiction field or an actor
// field, so actor_id is threaded through explicitly from each handler's
...
func (p *KafkaPublisher) Publish(ctx context.Context, params PublishParams) error {
	evt := Event{
		...
		EventVersion:  "1.0",
		SourceService: "connectivity-api-bridge-svc",
