...
// domain.ApiBridge carries a real LegalEntityID; domain.IngestionLog does
// not, so legal_entity_id is correctly omitted for ingestion events.
package events_test
...
func TestPublish_ActuallyWritesToKafka(t *testing.T) {
	w := &fakeWriter{}
	p := events.NewKafkaPublisherWithWriter(w, "zoiko.connectivity-api-bridge.events", zap.NewNop())

	err := p.Publish(context.Background(), events.PublishParams{
		EventType: "connectivity.bridge.created", AggregateID: "bridge-1", TenantID: "tenant-1",
	...
	assert.Equal(t, "connectivity.bridge.created", env.EventType)
	assert.Equal(t, "1.0", env.EventVersion)
	assert.Equal(t, "connectivity-api-bridge-svc", env.SourceService)
...
func TestPublish_RepeatEventsOnSameAggregate_GetDistinctEventIDs(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		err := p.Publish(context.Background(), events.PublishParams{
			EventType: "connectivity.bridge.ingested", AggregateID: "log-1", TenantID: "tenant-1",
