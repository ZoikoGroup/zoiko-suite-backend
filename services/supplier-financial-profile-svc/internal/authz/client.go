// Package authz is a real HTTP client to authorization-svc — including
// its dynamic own-object Segregation-of-Duties layer. This service is
// the first real caller of that feature outside authorization-svc's own
// tests: CheckAllowedOwnObject supplies resource_owner_principal_id so a
// proposer approving their own high-risk supplier-profile change is
// genuinely denied by authorization-svc's own SoD rule evaluation, not
// by logic duplicated here.
package authz
...
// CheckAllowedOwnObject is CheckAllowed plus resourceOwnerPrincipalID —
