...
// MigrationJob carries a real LegalEntityID but no jurisdiction field.
package events_test
...
func TestPublish_EnvelopeCarriesLegalEntityAndActor(t *testing.T) {
	...
	err := p.Publish(context.Background(), events.PublishParams{
		EventType: "migration.integrity_validated", SubjectID: "job-1", TenantID: "tenant-1",
		LegalEntityID: "entity-1", ActorID: "validator-1", CorrelationID: "corr-1",
	...
	assert.Equal(t, "migration.integrity_validated", env.EventType)
	assert.Equal(t, "1.0", env.EventVersion)
	assert.Equal(t, "migration-integrity-svc", env.SourceService)
...
func TestPublish_RepeatEventsOnSameJob_GetDistinctEventIDs(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		err := p.Publish(context.Background(), events.PublishParams{
			EventType: "migration.job_archived", SubjectID: "job-1", TenantID: "tenant-1",
