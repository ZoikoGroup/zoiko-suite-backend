...
// Tenant-scope tests for migration-integrity-svc (Priority 1b).
//
...
func TestMissingTenantHeader_Refused(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"Post /validate", http.MethodPost, "/v1/migrations/validate"},
		{"Get /", http.MethodGet, "/v1/migrations/"},
		{"Get /some-id", http.MethodGet, "/v1/migrations/some-id"},
		{"Post /some-id/audit/e1/remediate", http.MethodPost, "/v1/migrations/some-id/audit/e1/remediate"},
		{"Delete /some-id", http.MethodDelete, "/v1/migrations/some-id"},
	} {
