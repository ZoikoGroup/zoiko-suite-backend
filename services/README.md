...
| `intercompany-accounting-svc` | 8104 | Phase 3 — governed intercompany entries, matching logic, and balancing integrity (`/v1/intercompany-entries`); PENDING splits into MATCHED or MISMATCHED as the system-detected outcome of a single link-target action (not a separate human choice), and MISMATCHED can later resolve to MATCHED; both the source and target journal are verified independently against real FINALIZED journals in general-ledger-svc before an entry is ever persisted as MATCHED; source and target legal entities are always kept as two distinct fields, never netted into one position; every mutating action gated through authorization-svc, real HTTP client from day one; Postgres RLS + explicit `tenant_id` filters in every query from day one |
