...
| `performance-review-svc` | 8103 | Phase 4 — authoritative owner of review cycles and employee performance reviews (`/v1/review-cycles`, `/v1/reviews`); state machine INITIATED → SELF_ASSESSMENT_PENDING → MANAGER_REVIEW_PENDING → SUBMITTED → APPROVED → COMPLETED; employee.terminated events cancel open reviews; COMPLETED writes governance_decision_id; every mutating action gated through authorization-svc (fail-closed); Postgres RLS + explicit tenant_id filters; publishes review.cycle.created, review.created, review.completed to zoiko.workforce.events |
