...
func TestStream_DoesNotBlockTheCaller(t *testing.T) {
	...
	c := siem.New(upstream.URL, "key-management-svc", zap.NewNop())
	defer c.Close()

	start := time.Now()
	c.Stream(context.Background(), testTenant, "key.disabled", siem.SeverityHigh, "denied")
...
func TestStream_DoesNotUseTheRequestContext(t *testing.T) {
	...
	c.Stream(ctx, testTenant, "key.disabled", siem.SeverityHigh, "denied")
...
func TestStream_DisabledStartsNothing(t *testing.T) {
	c := siem.New("", "key-management-svc", zap.NewNop())
...
func TestStream_NoTenantIsANoOp(t *testing.T) {
	...
	c := siem.New(upstream.URL, "key-management-svc", zap.NewNop())
	c.Stream(context.Background(), "", "key.disabled", siem.SeverityHigh, "denied")
	c.Close()
}
