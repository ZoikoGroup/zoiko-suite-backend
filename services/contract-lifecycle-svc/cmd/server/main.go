...
import (
	...
	"zoiko.io/contract-lifecycle-svc/internal/config"
	svcenvelope "zoiko.io/contract-lifecycle-svc/internal/envelope"
...
func main() {
	logger, err := telemetry.NewLogger("contract-lifecycle-svc")
	if err != nil {
	...
	go func() {
		logger.Info("starting contract-lifecycle-svc", zap.String("port", cfg.Port))
	...
	logger.Info("shutting down contract-lifecycle-svc gracefully...")
