...
// service, and payload schema version. domain.ReviewRecord carries real
// TenantID/LegalEntityID and a CreatedByPrincipalID actor for the created
// event; completed uses the completing handler's already-verified
// principalID rather than ReviewerPrincipalID, since whoever marks a
// review complete is not necessarily the reviewer of record. No
...
func (p *Publisher) PublishReviewCreated(ctx context.Context, r domain.ReviewRecord) {
	p.emit(ctx, "review.created", r.CorrelationID, r.TenantID, r.LegalEntityID, r.CreatedByPrincipalID, r.ReviewID, map[string]any{
...
func (p *Publisher) PublishReviewCompleted(ctx context.Context, actorID string, r domain.ReviewRecord) {
	p.emit(ctx, "review.completed", r.CorrelationID, r.TenantID, r.LegalEntityID, actorID, r.ReviewID, map[string]any{
		"review_id":       r.ReviewID,
		"legal_entity_id": r.LegalEntityID,
		"cycle_id":        r.CycleID,
		"employee_id":     r.EmployeeID,
