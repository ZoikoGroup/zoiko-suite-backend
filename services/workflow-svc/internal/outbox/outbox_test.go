...
func (m *mockConditionalPublisher) PublishOutbox(ctx context.Context, outboxEventID, eventType, correlationID, tenantID, legalEntityID, actorID string, payload []byte) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func TestRelay_MaxAttemptsCeiling_QueryFilterAndDeadLetterLogging(t *testing.T) {
	eventID := uuid.NewString()
	envBytes := []byte(`{"event_type":"workflow.created"}`)
	aID := "a-1"
	cID := "corr-1"
