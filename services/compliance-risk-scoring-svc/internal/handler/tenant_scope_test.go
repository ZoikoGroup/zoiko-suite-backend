...
// Tenant-scope tests for compliance-risk-scoring-svc (Priority 1b).
//
...
func TestMissingTenantHeader_Refused(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"Post /calculate", http.MethodPost, "/v1/risk-scores/calculate"},
		{"Get /", http.MethodGet, "/v1/risk-scores/"},
		{"Get /thresholds", http.MethodGet, "/v1/risk-scores/thresholds"},
		{"Post /thresholds", http.MethodPost, "/v1/risk-scores/thresholds"},
		{"Get /some-id", http.MethodGet, "/v1/risk-scores/some-id"},
		{"Delete /some-id", http.MethodDelete, "/v1/risk-scores/some-id"},
	} {
