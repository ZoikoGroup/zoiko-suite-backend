...
// RiskScoreAssessment carries a real LegalEntityID but no jurisdiction
// field.
package events_test
...
func TestPublish_EnvelopeCarriesLegalEntityAndActor(t *testing.T) {
	w := &fakeWriter{}
	p := events.NewPublisherWithWriter(w, "zoiko.compliance-risk-scoring.events", zap.NewNop())

	err := p.Publish(context.Background(), events.PublishParams{
		EventType: "risk_score.calculated", SubjectID: "assess-1", TenantID: "tenant-1",
		LegalEntityID: "entity-1", ActorID: "assessor-1", CorrelationID: "corr-1",
		Payload: map[string]string{"risk_tier": "HIGH"},
	})
	require.NoError(t, err)
	require.Len(t, w.msgs, 1)

	env := decode(t, w.msgs[0])
	assert.Equal(t, "risk_score.calculated", env.EventType)
	assert.Equal(t, "1.0", env.EventVersion)
	assert.Equal(t, "compliance-risk-scoring-svc", env.SourceService)
	assert.Equal(t, "entity-1", env.LegalEntityID)
	assert.Equal(t, "assessor-1", env.ActorID)
	assert.Equal(t, "corr-1", env.CorrelationID)
	assert.NotEmpty(t, env.EventID)
}

func TestPublish_RepeatEventsOnSameAssessment_GetDistinctEventIDs(t *testing.T) {
	w := &fakeWriter{}
	p := events.NewPublisherWithWriter(w, "zoiko.compliance-risk-scoring.events", zap.NewNop())

	for i := 0; i < 2; i++ {
		err := p.Publish(context.Background(), events.PublishParams{
			EventType: "risk_assessment.archived", SubjectID: "assess-1", TenantID: "tenant-1",
