...
// Config holds all runtime configuration for intercompany-accounting-svc.
type Config struct {
	Env  string
	Port int
	...
	// LedgerServiceURL is the base URL of general-ledger-svc. Linking a
	// target journal verifies BOTH the source and target journal_ids are
	// real FINALIZED journals there — status, legal entity, and net amount
	// all cross-checked on each side — before an entry is allowed to
...
func Load() (*Config, error) {
	return &Config{
		Env: env("ENV", "local"),
		// 8104: 8080-8099 are already taken by every other service built so
		// far (accounts-payable-svc claimed 8099) — see services/README.md.
		Port: envInt("PORT", 8104),
		DB: DBConfig{
			Host:     env("DB_HOST", "localhost"),
			Port:     envInt("DB_PORT", 5432),
			Name:     env("DB_NAME", "intercompany_accounting"),
		...
		Kafka: KafkaConfig{
			...
			Topic:   env("KAFKA_EVENTS_TOPIC", "zoiko.intercompany-accounting.events"),
		},
