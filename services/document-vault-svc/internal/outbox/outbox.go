...
// document-vault-svc per ZS-STATE-001 Invariant I-13.
//
// An event is written to outbox_events inside the SAME database transaction
// as the authoritative document mutation, guaranteeing state mutations and
...
// delivery to Kafka. Mirrors accounts-payable-svc/accounts-receivable-svc/
// general-ledger-svc/payment-authorization-svc/workflow-svc's own outbox
// packages exactly, so the platform has one outbox implementation, not six
// slightly different ones.
//
// Known platform-wide gap, not specific to this copy: the relay retries a
// failing publish forever on a fixed interval with no max-attempt ceiling
// or dead-letter path. Not fixed here to avoid this one service silently
// diverging from the other five — track/fix it platform-wide instead.
package outbox
...
// VariantAEnvelope preserves the platform's existing Kafka event contract.
// Notice: There is intentionally NO event_id in the JSON body (Variant A
// contract). Event ID is conveyed via the X-Event-ID Kafka header.
...
// NewVariantAEnvelope constructs a Variant A envelope matching the platform specification.
func NewVariantAEnvelope(eventType, correlationID, tenantID, legalEntityID, actorID string, payload any) (VariantAEnvelope, error) {
	...
	return VariantAEnvelope{
		...
		SchemaVersion: "1.0",
		SourceService: "document-vault-svc",
