...
//
// Only real GRANTED/DENIED decisions are ever cached (see CheckAllowed).
// An unreachable or misbehaving authorization-svc is never cached — that
// would turn one transient outage into a standing permit-or-deny for
// every subsequent caller on this instance, which defeats fail-closed.
// A cache hit also does not produce a new Decision Logging Layer entry at
// authorization-svc; the original live check that populated the cache
// entry already recorded that decision as evidence.
const decisionCacheTTL = 5 * time.Second

type cachedDecision struct {
	// deniedErr is nil for GRANTED, domain.ErrAuthorizationDenied for
	// DENIED — never any other value.
	deniedErr error
	expiresAt time.Time
}
