...
func Load() (*Config, error) {
	return &Config{
		Env:  env("ENV", "local"),
		Port: envInt("PORT", 8109),
		DB: DBConfig{
			Host:     env("DB_HOST", "localhost"),
			Port:     envInt("DB_PORT", 5432),
			Name:     env("DB_NAME", "employment_contracts"),
		...
		Kafka: KafkaConfig{
			...
			GroupID: env("KAFKA_GROUP_ID", "employment-contracts-svc"),
			Topic:   env("KAFKA_EVENTS_TOPIC", "zoiko.contract.events"),
		},
