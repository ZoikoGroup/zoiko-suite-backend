...
// write from the handler after the commit — so a broker hiccup during a
// revocation discarded the only notice that an authority had been withdrawn,
...
const (
	...
	//
	// 250ms rather than a second because of what the events are: authority
	// .revoked is a session-invalidation signal, and the gap between an
	// operator being told a revocation succeeded and the delegate's session
	// actually ending is this number.
