...
// here. EVERY write on this service calls authorization-svc twice — once to
// authorize the caller, once to provision the definition through its admin API
// — and this service fails closed on both. With authorization-svc unreachable,
// a healthy database means every write returns 503 while the container reports
// ready and the load balancer keeps sending traffic to it. A probe that cannot
// distinguish "serving" from "answering 503 promptly" is not a readiness probe.
//
...
//
// Reads are deliberately NOT a reason to stay ready on their own. The catalogue
// is readable with authorization-svc down — identity-context-svc depends on
// that — but an instance that can only be read from is not an instance that can
// serve this service's purpose, which is authoring.
