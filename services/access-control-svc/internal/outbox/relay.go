...
// allowed to be lost. Before this existed the two were the same act — a Kafka
// write from the handler after the commit, with the error logged and discarded
// — so a broker hiccup during a role retirement threw away the only notice that
// the role had changed and reported success.
package outbox
...
const (
	...
	//
	// 250ms rather than a second because of what the events are: role.updated
	// is a session-invalidation signal, and the gap between an operator being
	// told a role was retired and the sessions carrying its grants actually
	// ending is this number plus the consumer's own lag.
