// Package clients holds inventory-management-svc's outbound HTTP clients
...
// CheckPeriodOpen is INV-03's own "hard-closed-period" dependency on
// financial-close-svc — the spec's own negative path, "Hard-closed
// movement backdated without correction path." Mirrors
// asset-management-svc's own internal/clients.Clients.CheckPeriodOpen
// exactly: a period financial-close-svc has never heard of defaults to
// open (this service does not own the calendar), and an unreachable
// financial-close-svc fails CLOSED, never silently open.
func (c *Clients) CheckPeriodOpen(ctx context.Context, tenantID, legalEntityID, periodName string) error {
	...
	if err != nil {
		c.log.Error("financial-close-svc unreachable — failing closed on movement commit", zap.Error(err))
