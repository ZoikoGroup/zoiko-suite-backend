...
// ReportDefinition carries a real LegalEntityID; domain.ReportRun does
// not, so legal_entity_id is correctly omitted for run events.
package events_test
...
func TestPublish_EnvelopeCarriesLegalEntityAndActor(t *testing.T) {
	...
	err := p.Publish(context.Background(), events.PublishParams{
		EventType: "report.definition_created", SubjectID: "def-1", TenantID: "tenant-1",
	...
	assert.Equal(t, "report.definition_created", env.EventType)
	assert.Equal(t, "1.0", env.EventVersion)
	assert.Equal(t, "reporting-orchestration-svc", env.SourceService)
...
func TestPublish_RepeatEventsOnSameRun_GetDistinctEventIDs(t *testing.T) {
	w := &fakeWriter{}
	p := events.NewPublisherWithWriter(w, zap.NewNop())

	for i := 0; i < 2; i++ {
		err := p.Publish(context.Background(), events.PublishParams{
			EventType: "report.run_completed", SubjectID: "run-1", TenantID: "tenant-1",
			ActorID: "trigger-1", CorrelationID: "corr-x",
