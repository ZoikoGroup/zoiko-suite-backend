...
// domain.BankConnection carries a real LegalEntityID; domain.BankStatement
// does not, so legal_entity_id is correctly omitted for statement events.
package events_test
...
func TestPublish_ActuallyWritesToKafka(t *testing.T) {
	w := &fakeWriter{}
	p := events.NewKafkaPublisherWithWriter(w, "zoiko.banking-connector.events", zap.NewNop())

	err := p.Publish(context.Background(), events.PublishParams{
		EventType: "banking.connection.created", AggregateID: "conn-1", TenantID: "tenant-1",
	...
	assert.Equal(t, "banking.connection.created", env.EventType)
	assert.Equal(t, "1.0", env.EventVersion)
	assert.Equal(t, "banking-connector-svc", env.SourceService)
...
func TestPublish_RepeatEventsOnSameAggregate_GetDistinctEventIDs(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		err := p.Publish(context.Background(), events.PublishParams{
			EventType: "banking.statement.ingested", AggregateID: "stmt-1", TenantID: "tenant-1",
