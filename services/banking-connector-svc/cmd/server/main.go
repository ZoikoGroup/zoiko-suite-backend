...
import (
	...
	"zoiko.io/banking-connector-svc/internal/handler"
	"zoiko.io/banking-connector-svc/internal/health"
	"zoiko.io/banking-connector-svc/internal/middleware"
	"zoiko.io/banking-connector-svc/internal/mtls"
	"zoiko.io/banking-connector-svc/internal/store"
	"zoiko.io/banking-connector-svc/internal/telemetry"
)
...
func main() {
	...
	cfg := config.Load()
	logger.Info("starting banking-connector-svc", zap.String("port", cfg.Port))
	...
	if cfg.AuthzMTLSEnabled {
		mtlsHTTPClient, err := mtls.NewClientHTTPClient(ctx, cfg.MTLSManagementServiceURL, "banking-connector-svc", platformScopeID)
		if err != nil {
	...
	logger.Info("banking-connector-svc running", zap.String("addr", srv.Addr))
	<-stop

	logger.Info("shutting down banking-connector-svc...")
	...
	logger.Info("banking-connector-svc stopped cleanly")
}
