...
// fields Doc 03 §19 requires. domain.FilingDraft carries real
...
func TestPublish_EnvelopeCarriesJurisdictionAndActor(t *testing.T) {
	...
	err := p.Publish(context.Background(), events.PublishParams{
		EventType: "filing.ready.for.submission", DraftID: "draft-1", TenantID: "tenant-1",
		LegalEntityID: "entity-1", Jurisdiction: "jurisdiction-1", ActorID: "preparer-1", CorrelationID: "corr-1",
	...
	assert.Equal(t, "filing.ready.for.submission", env.EventType)
	assert.Equal(t, "1.0", env.EventVersion)
	assert.Equal(t, "filing-preparation-svc", env.SourceService)
...
func TestPublish_RepeatEventsOnSameDraft_GetDistinctEventIDs(t *testing.T) {
	w := &fakeWriter{}
	p := events.NewKafkaPublisherWithWriter(w, zap.NewNop())

	for i := 0; i < 2; i++ {
		err := p.Publish(context.Background(), events.PublishParams{
			EventType: "filing.draft.updated", DraftID: "draft-1", TenantID: "tenant-1",
