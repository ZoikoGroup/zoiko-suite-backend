...
// Event is the canonical fact emitted by filing-preparation-svc.
...
type Publisher interface {
	Publish(ctx context.Context, eventType, draftID, tenantID string, payload interface{}) error
}
...
func (p *KafkaPublisher) Publish(ctx context.Context, eventType, draftID, tenantID string, payload interface{}) error {
	evt := Event{
		EventID:    "evt-" + eventType + "-" + draftID,
		EventType:  eventType,
		DraftID:    draftID,
		TenantID:   tenantID,
