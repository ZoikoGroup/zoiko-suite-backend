...
//
// TenantID/Jurisdiction are `omitempty`, not fabricated defaults: both
// domain.RetentionPolicy and domain.LegalHold have independently-nullable
// TenantID (a platform-wide policy/hold has none), and RetentionPolicy
// additionally has a real JurisdictionCode when one applies.
// LegalEntityID is correctly omitted everywhere — neither struct has one.
type Event struct {
