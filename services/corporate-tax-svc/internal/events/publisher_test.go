...
// fields Doc 03 §19 requires. domain.TaxReturn carries real
...
func TestPublish_EnvelopeCarriesJurisdictionAndActor(t *testing.T) {
	...
	err := p.Publish(context.Background(), events.PublishParams{
		EventType: "corporate_tax_return.submitted", ReturnID: "return-1", TenantID: "tenant-1",
		LegalEntityID: "entity-1", Jurisdiction: "jurisdiction-1", ActorID: "submitter-1", CorrelationID: "corr-1",
	...
	assert.Equal(t, "corporate_tax_return.submitted", env.EventType)
	assert.Equal(t, "1.0", env.EventVersion)
	assert.Equal(t, "corporate-tax-svc", env.SourceService)
...
func TestPublish_RepeatEventsOnSameReturn_GetDistinctEventIDs(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		err := p.Publish(context.Background(), events.PublishParams{
			EventType: "corporate_tax_return.updated", ReturnID: "return-1", TenantID: "tenant-1",
