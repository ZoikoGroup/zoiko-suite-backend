...
// domain.TaxInterface carries real LegalEntityID and Jurisdiction;
// domain.TaxFilingSubmission does not, so both are sourced from the
// interface for submission events.
package events_test
...
func TestPublish_ActuallyWritesToKafka(t *testing.T) {
	w := &fakeWriter{}
	p := events.NewKafkaPublisherWithWriter(w, "zoiko.tax-authority-interface.events", zap.NewNop())

	err := p.Publish(context.Background(), events.PublishParams{
		EventType: "tax.interface.created", AggregateID: "iface-1", TenantID: "tenant-1",
		LegalEntityID: "entity-1", Jurisdiction: "uk-england", ActorID: "creator-1", CorrelationID: "corr-1",
	...
	assert.Equal(t, "tax.interface.created", env.EventType)
	assert.Equal(t, "1.0", env.EventVersion)
	assert.Equal(t, "tax-authority-interface-svc", env.SourceService)
...
func TestPublish_RepeatEventsOnSameAggregate_GetDistinctEventIDs(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		err := p.Publish(context.Background(), events.PublishParams{
			EventType: "tax.filing.submitted", AggregateID: "sub-1", TenantID: "tenant-1",
			ActorID: "submitter-1", CorrelationID: "corr-x",
