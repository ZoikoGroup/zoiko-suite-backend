...
// service, and payload schema version. domain.TaxInterface carries real
// LegalEntityID and Jurisdiction fields; domain.TaxFilingSubmission does
// not (it links back to an interface by ID only), so both are correctly
// omitted for submission events unless the caller resolves them from the
// interface. Neither domain object has an actor field, so actor_id is
// threaded through explicitly from each handler's already-verified
// principalID.
type Event struct {
...
func (p *KafkaPublisher) Publish(ctx context.Context, params PublishParams) error {
	evt := Event{
		...
		EventVersion:  "1.0",
		SourceService: "tax-authority-interface-svc",
