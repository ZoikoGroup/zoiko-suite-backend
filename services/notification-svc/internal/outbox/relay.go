...
// the same transaction as the delivery conclusion it describes, and this
// package decides when it is DELIVERED. Delivery is allowed to fail and be
// retried; the fact is not allowed to be lost.
//
// Before this existed the two were the same act — a Kafka write from the
// handler and from the retry worker after the commit, with the error logged and
// discarded. A broker hiccup at the moment a notice concluded therefore threw
// away the only notice that a governed notification had gone out (or had
// definitively not), answered 201, and left a register showing a perfectly
// healthy row. Nothing reported it, because the thing that would have reported
// it was the event that was lost.
package outbox
...
const (
	// DefaultBatchSize is how many events one drain claims. Large enough that
	// a backlog clears in few round trips, small enough that a failing batch
	// does not hold a long write lock on the outbox.
	...
	//
	// One second rather than the 250ms configuration-feature-flag-svc uses,
	// because of what the events are for. A feature flag event is a rollout
	// control, where this number plus consumer lag is how long a feature stays
	// on after an operator turns it off. A notification event is a record that
	// a notice was issued — consumed by escalation chains and audit sinks that
	// act on timescales of minutes. Paying four times the idle database
	// round-trips to shave 750ms off that would buy nothing.
