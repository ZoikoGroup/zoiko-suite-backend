...
import (
	...
	"zoiko.io/kill-switch-registry-svc/internal/authz"
	"zoiko.io/kill-switch-registry-svc/internal/config"
	"zoiko.io/kill-switch-registry-svc/internal/events"
	"zoiko.io/kill-switch-registry-svc/internal/handler"
...
func main() {
	logger, err := telemetry.NewLogger("kill-switch-registry-svc")
	if err != nil {
	...
	if cfg.AuthzMTLSEnabled {
		mtlsHTTPClient, err := mtls.NewClientHTTPClient(ctx, cfg.MTLSManagementServiceURL, "kill-switch-registry-svc", platformScopeID)
		if err != nil {
	...
	go func() {
		logger.Info("starting kill-switch-registry-svc", zap.String("port", cfg.Port))
