...
func Load() (*Config, error) {
	return &Config{
		Env:  env("ENV", "local"),
		Port: envInt("PORT", 8107),
		DB: DBConfig{
			Host:     env("DB_HOST", "localhost"),
			Port:     envInt("DB_PORT", 5432),
			Name:     env("DB_NAME", "invoice_approval"),
		...
		Kafka: KafkaConfig{
			...
			GroupID: env("KAFKA_GROUP_ID", "invoice-approval-svc"),
			Topic:   env("KAFKA_EVENTS_TOPIC", "zoiko.invoice-approval.events"),
		},
