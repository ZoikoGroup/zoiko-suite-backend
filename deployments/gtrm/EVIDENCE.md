...
| O | Token/hostname mismatch | **Out of GTRM scope, now closed at its assigned owner.** §14 assigns this to the application/auth layer. `emit.go` now also stamps `X-Zoiko-Resolved-Tenant-Id` (the canonical `tenant_id`, alongside the existing slug) into the trusted per-tenant context — stripped from client input like every other GTRM header. `gateway-auth-svc.Verify` rejects (403 `tenant_hostname_mismatch`) any validly-signed token whose `tenant_id` claim disagrees with that header when present; absence of the header (routes GTRM hasn't onboarded yet) is a no-op, not a fail-closed block. See `services/gateway-auth-svc/internal/handler/handler.go` and its tests. | ✅ UNIT (gateway-auth-svc) |
